...
export function llmsTxt(base: string): string {
  return [
    ...
    "## API",
    "",
    `- [POST /api/diff](${base}/llms-full.txt): Create an encrypted diff — send JSON with encrypted_data containing ciphertext, iv, and salt; every share previews at /s/<id>`,
    `- [POST /api/files](${base}/llms-full.txt): Create an encrypted file bundle — same format; every share previews at /s/<id>`,
    "",
    "## Optional",
    "",
...
export function llmsFullTxt(base: string): string {
  return [
    ...
    "from the decrypted payload. The older kind-specific links (`/p/`, `/f/`, `/app/`, `/plan/`,",
    "`/d/`) still resolve and always will, so any URL previously handed out keeps working; they",
    "are simply no longer minted for new shares.",
    "",
    "### GET /api/share/<id>",
    "",
    ...
    "`agentgate:` URL. The app viewer inlines the vendored copy from the server into the sandbox",
    "before rendering, so it costs nothing in the payload.",
    "",
    ...
    "`/static/vendor/<real-filename>`. Anything not in the tables above is left untouched: a",
    "bundle-local path resolves from the uploaded files, and a remote URL stays remote.",
    "",
    "## Encryption Details",
    "",
    ...
    "5. Present the public share URL (`/s/<id>`) to the user",
